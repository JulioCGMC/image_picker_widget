...
class ImagePickerWidget extends StatefulWidget {
  ...
  @override
  _ImagePickerWidgetState createState() => _ImagePickerWidgetState(this.initialImage);
}
...
class _ImagePickerWidgetState extends State<ImagePickerWidget> {
  ...
  Widget _editMode({@required Widget child}) {
    if (widget.isEditable) return GestureDetector(
      onTap: () {
        changeImage(
          context,
          ModalImageSelector(
  ...
  DecorationImage _image() {
    if (image is File) return DecorationImage(
      image: FileImage(image),
      fit: BoxFit.cover
    );
    else if (image != null) return DecorationImage(
